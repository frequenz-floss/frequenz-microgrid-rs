...
use super::{
    AggregationFormula, LogicalMeterConfig, Metric, logical_meter_actor::LogicalMeterActor,
};
...
impl LogicalMeterHandle {
    ...
    /// connection point.
    pub fn grid(&mut self, metric: Metric) -> Result<AggregationFormula, Error> {
    ...
    pub fn consumer(&mut self, metric: Metric) -> Result<AggregationFormula, Error> {
        if !metric.power() && !metric.current() {
            return Err(Error::invalid_metric(format!(
    ...
    pub fn producer(&mut self, metric: Metric) -> Result<AggregationFormula, Error> {
        if !metric.power() && !metric.current() {
            return Err(Error::invalid_metric(format!(
